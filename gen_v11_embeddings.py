"""Generate v11 embeddings from AppraiseHate v11 outputs."""
...
MECHANISMS = [
    "stereotype",
    "sexism_objectification",
    "sexual_shame_purity",
    "group_comparison",
    "moral_condemnation",
    "offensive_humor_irony",
    "exclusion",
    "dehumanization",
    "threat_framing",
    "violent_legitimization",
    "none",
]
...
def extract_struct(samples):
    if not samples:
        return torch.zeros(7 * 4 + len(STANCES) + 2 + len(MECHANISMS) + 3)

    per_key = {k: [] for k in APPRAISAL_KEYS}
    stance_ids = []
    quoted_vals = []
    target_vals = []
    mech_vals = {k: [] for k in MECHANISMS}
    ...
    for sample in samples:
        ...
        sample_mechs = sample.get("implicit_hate_mechanisms", []) or ["none"]
        if not isinstance(sample_mechs, list):
            sample_mechs = ["none"]
        sample_mechs = {
            mechanism for mechanism in sample_mechs if mechanism in MECHANISMS
        } or {"none"}
        for mechanism in MECHANISMS:
            mech_vals[mechanism].append(1.0 if mechanism in sample_mechs else 0.0)
    ...
    features.extend(float(np.mean(mech_vals[mechanism])) for mechanism in MECHANISMS)
...
def main():
    ...
    if args.dataset_name == "HateMM":
        data_path = "./datasets/HateMM/appraise_v11_data.json"
        emb_dir = "./embeddings/HateMM"
    else:
        data_path = f"./datasets/Multihateclip/{args.language}/appraise_v11_data.json"
    ...
    for i, item in enumerate(data):
        ...
        cues = sample0.get("surface_cues", "") or ""
        stance = sample0.get("stance", "unclear") or "unclear"
        quoted = bool(sample0.get("quoted_or_criticized", False))
        mechanisms = sample0.get("implicit_hate_mechanisms", []) or ["none"]
        if not isinstance(mechanisms, list):
            mechanisms = ["none"]
        mechanisms = [m for m in mechanisms if m in MECHANISMS] or ["none"]
    ...
    torch.save(t1_features, f"{emb_dir}/v11_t1_features.pth")
    torch.save(t2_features, f"{emb_dir}/v11_t2_features.pth")
    torch.save(score_features, f"{emb_dir}/v11_scores.pth")
    torch.save(struct_features, f"{emb_dir}/v11_struct_features.pth")

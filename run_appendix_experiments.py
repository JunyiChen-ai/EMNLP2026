"""
Appendix experiments for all 4 datasets under max performance config.
Generates all missing data and stores results to JSON.

Experiments:
1. Statistical significance (paired bootstrap + McNemar) vs strongest baseline
2. Seed stability (5 seeds, mean±std)
3. Calibration (ECE, Brier before/after retrieval)
4. Bank size curve (5%-100%)
5. Modality zero-out robustness
6. Label noise robustness (0-20%)
7. Per-class P/R/F1 + confusion matrix

All under max performance config (exact seed + retrieval params).
"""
...
def apply_retrieval(cfg,tp,tl_arr,tla,vp,vl_arr,vla,tep,tel_arr,tela,nc=2):
    blt=torch.tensor(tla)
    if cfg["whiten"]=="zca": tr_w,va_w,te_w=zca_whiten(tp,vp,tep)
    elif cfg["whiten"].startswith("spca"):
        r=int(cfg["whiten"].split("_r")[1]);tr_w,va_w,te_w=spca_whiten(tp,vp,tep,r=r)
    ...
    return preds,bl_test,tr_w,te_w,blt
...
def main():
    ...
    for ds_name, cfg in BEST_CONFIGS.items():
        print(f"\n{'='*60}\n  {ds_name}\n{'='*60}")
        out_dir=f"./appendix_results/{ds_name}"
        ...
        lm=cfg["label_map"];nc=2;seed=cfg["seed"]
        results={}
        ...
        ours_preds,bl_test,tr_w,te_w,blt=apply_retrieval(cfg,tp,tl_arr,tla,vp,vl_arr,vla,tep,tel_arr,tela)
        ours_acc=accuracy_score(tela,ours_preds)
        print(f"  Ours: ACC={ours_acc:.4f}")

        # Train HVGuard baseline
        print("  Training HVGuard baseline...")
        torch.manual_seed(seed);random.seed(seed);np.random.seed(seed)
        fk_base=list(mk_base);common_b=set.intersection(*[set(feats[k].keys()) for k in fk_base])&set(feats["labels"].keys())
        ...
        for e in range(20):
            ...
            for batch in trl_b:
                ...
                crit_h(hvg(torch.cat([batch[k] for k in mk_base],dim=-1)),batch["label"]).backward()
                opt_h.step()
                with torch.no_grad():
            ...
            with torch.no_grad():
                for batch in vl_b:
                    batch={k:v.to(device) for k,v in batch.items()}
                    ps.extend(hvg_ema(torch.cat([batch[k] for k in mk_base],dim=-1)).argmax(1).cpu().numpy())
                    ls.extend(batch["label"].cpu().numpy())
            va=accuracy_score(ls,ps)
        ...
        with torch.no_grad():
            for batch in tel_b:
                batch={k:v.to(device) for k,v in batch.items()}
                hvg_preds.extend(hvg_ema(torch.cat([batch[k] for k in mk_base],dim=-1)).argmax(1).cpu().numpy())
                hvg_labels.extend(batch["label"].cpu().numpy())
        hvg_preds=np.array(hvg_preds);hvg_labels=np.array(hvg_labels)
        print(f"  HVGuard: ACC={accuracy_score(hvg_labels,hvg_preds):.4f}")

        # ===== 1. Statistical Significance =====
        print("  1. Statistical significance...")
        n=len(tela);np.random.seed(42)
        delta_accs,delta_f1s=[],[]
        for _ in range(10000):
            idx=np.random.choice(n,n,replace=True)
            da=accuracy_score(tela[idx],ours_preds[idx])-accuracy_score(tela[idx],hvg_preds[idx][:n])
            df=f1_score(tela[idx],ours_preds[idx],average='macro')-f1_score(tela[idx],hvg_preds[idx][:n],average='macro')
            delta_accs.append(da);delta_f1s.append(df)
        da=np.array(delta_accs);df=np.array(delta_f1s)
        b=((hvg_preds[:n]==tela)&(ours_preds!=tela)).sum()
        c=((ours_preds==tela)&(hvg_preds[:n]!=tela)).sum()
        chi2=(abs(b-c)-1)**2/(b+c) if b+c>0 else 0
        from scipy.stats import chi2 as chi2_dist
        p_mc=1-chi2_dist.cdf(chi2,1) if b+c>0 else 1.0
        results["significance"]={
            "delta_acc_mean":float(da.mean()),"delta_acc_ci":[float(np.percentile(da,2.5)),float(np.percentile(da,97.5))],
            "delta_acc_p":float(max((da<=0).mean(),1/10001)),
            "delta_f1_mean":float(df.mean()),"delta_f1_ci":[float(np.percentile(df,2.5)),float(np.percentile(df,97.5))],
            "delta_f1_p":float(max((df<=0).mean(),1/10001)),
            "mcnemar_b":int(b),"mcnemar_c":int(c),"mcnemar_chi2":float(chi2),"mcnemar_p":float(p_mc),
        }
        print(f"    ΔACC={da.mean()*100:.2f}% CI=[{np.percentile(da,2.5)*100:.2f},{np.percentile(da,97.5)*100:.2f}] p={max((da<=0).mean(),1/10001):.4f}")
        ...
        for s in [42,1042,2042,3042,4042]:
            ...
            try:
                preds_r,_,_,_,_=apply_retrieval(cfg,tp_s,tl_s,tla_s,vp_s,vl_s,vla_s,tep_s,tel_s,tela_s)
                full_acc=accuracy_score(tela_s,preds_r);full_f1=f1_score(tela_s,preds_r,average='macro')
            ...
            print(f"    seed={s}: head={head_acc:.4f} full={full_acc:.4f}")
        ...
        for name,probs in [("head",head_probs),("full",blend_probs)]:
            ...
            results[f"calibration_{name}"]={"ece":float(ece),"brier":float(brier)}
        ...
        for frac in [0.05,0.10,0.25,0.50,0.75,1.0]:
            ...
            for trial in range(5):
                n_bank=int(len(tr_w)*frac)
                idx=np.random.choice(len(tr_w),n_bank,replace=False)
                sub_tr=tr_w[idx];sub_bl=blt[idx]
                knn_fn=cosine_knn if cfg["knn_type"]=="cosine" else csls_knn
                kt=knn_fn(te_w,sub_tr,sub_bl,k=min(cfg["k"],n_bank),nc=nc,temperature=cfg["temp"])
                fl=(1-cfg["alpha"])*tel_arr+cfg["alpha"]*kt
        ...
        # ===== 5. Modality Zero-out =====
        print("  5. Modality zero-out...")
        base_acc=accuracy_score(tela,head_preds)
        modality_drops={}
        for mod in mk_full:
            ps_z=[];ls_z=[]
            ema.eval()
            with torch.no_grad():
                for batch in DataLoader(DS(cur["test"],feats,lm,mk_full),64,False,collate_fn=collate_fn):
                    ...
                    lo=ema(batch);ps_z.extend(lo.argmax(1).cpu().numpy());ls_z.extend(batch["label"].cpu().numpy())
            drop_acc=accuracy_score(ls_z,ps_z)
            modality_drops[mod]={"acc":float(drop_acc),"drop":float(base_acc-drop_acc)}
            print(f"    {mod}: ACC={drop_acc:.4f} (drop={base_acc-drop_acc:+.4f})")
        ...
        for noise_rate in [0.0,0.05,0.10,0.20]:
            ...
            for trial in range(5):
                ...
                flip_idx=np.random.choice(len(noisy_bl),n_flip,replace=False)
                noisy_bl[flip_idx]=1-noisy_bl[flip_idx]
                knn_fn=cosine_knn if cfg["knn_type"]=="cosine" else csls_knn
                kt=knn_fn(te_w,tr_w,noisy_bl,k=cfg["k"],nc=nc,temperature=cfg["temp"])
                fl=(1-cfg["alpha"])*tel_arr+cfg["alpha"]*kt
                accs.append(accuracy_score(tela,np.argmax(fl,axis=1)))
        ...
        # ===== 7. Per-class =====
        print("  7. Per-class...")
        p,r,f,_=precision_recall_fscore_support(tela,ours_preds,average=None)
        cm=confusion_matrix(tela,ours_preds)
        results["per_class"]={"precision":p.tolist(),"recall":r.tolist(),"f1":f.tolist(),"cm":cm.tolist()}
        print(f"    Per-class F1: {f.tolist()}")

        # Save
        with open(f"{out_dir}/appendix_results.json","w") as f:json.dump(results,f,indent=2)

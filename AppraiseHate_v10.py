"""AppraiseHate v10: dual-axis prompt for explicit + implicit hate.

Key change from v9:
- Separate content-level hateful meaning from speaker stance.
- Preserve appraisal scores for quoted/reported/criticized hateful content.
- Add explicit `target_group` and `quoted_or_criticized` fields.

This keeps the HateMM reportage safeguard while reducing under-scoring on
implicit MultiHateClip cases.
"""
...
def setup_logger():
    ...
    log_file = f"./logs/appraise_v10_{ts}.log"
...
async def request_with_retries(messages, max_tokens=900, logger=None):
...
async def run_v10_prompt(video_frames, title, transcript, emotion, logger=None):
    prompt = f"""{QUAD_INTRO}
...
2. SPEAKER STANCE: does the video endorse it, merely report/quote it, condemn it, mock it, or remain unclear?

Use cognitive appraisal theory. Group-targeted hate often frames a target as:
- blameworthy
- threatening
...
Implicit hate counts when the video relies on stereotypes, derogatory comparison, moral inferiority, purity/sexual shame, "unnatural/sinful" framing, exclusion, or contempt toward a social group even without explicit slurs or direct violence.

Targeting rules:
- If an individual is attacked via a group-marked identity (e.g. woman, Muslim, Christian, gay, trans, Black), treat the broader group as targeted.
...
- News, education, and counter-speech should only score near zero when the hateful proposition is merely background and the video's own message clearly rejects or neutralizes it.

Scoring:
- 0 = absent
- 1 = implied / subtle / partial
- 2 = clear / central / strong / repeated
...
- Focus on implied social meaning, not just literal wording.
- Keep outputs short.
- In implicit_meaning, state the strongest group-targeted meaning first. If stance is not endorse, mention that the video is reporting/criticizing/mocking rather than endorsing.
- Return JSON only with exactly these 6 top-level fields:
...
async def process(data_path, save_path, quad_root, max_concurrent, logger, n_samples=3):
    ...
    done = sum(1 for d in data if len(d.get("v10_samples", [])) >= n_samples)
    ...
    pbar = tqdm(total=len(data), initial=done, desc="V10", unit="video")

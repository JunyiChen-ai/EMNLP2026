"""Train/evaluate with AppraiseHate v12 features.

v12 has no appraisal_vector scores — uses simplified AC-MHGF without FiLM conditioning.
Modalities: text + audio + frame + T1 + T2 + struct (no scores).
"""
...
def setup_logger():
    ...
    lf = f"./logs/main_v12_{ts}.log"
    logger = logging.getLogger("v12main"); logger.setLevel(logging.INFO); logger.handlers.clear()
...
class AC_MHGF_NoScores(nn.Module):
    """AC-MHGF without appraisal score FiLM conditioning.
    Uses learned per-modality parameters instead of sample-specific conditioning.
    """
    def __init__(self, num_mod=5, num_heads=4, hidden=192, struct_dim=9,
                 num_classes=2, dropout=0.15, mod_drop=0.15):
        super().__init__()
...
def main():
    ...
    logger.info(f"Data: {len(common)}, train={len(split_ids['train'])}, val={len(split_ids['valid'])}, test={len(split_ids['test'])}, struct_dim={struct_dim}")

    all_results = []
    for ri in range(args.num_runs):
        seed = ri * 1000 + 42; torch.manual_seed(seed); random.seed(seed); np.random.seed(seed)
        trd = V12Dataset(split_ids["train"], features, label_map)
        vd = V12Dataset(split_ids["valid"], features, label_map)
        ted = V12Dataset(split_ids["test"], features, label_map)
        ...
        model = AC_MHGF_NoScores(struct_dim=struct_dim, num_classes=args.num_classes).to(device)
        ema = copy.deepcopy(model)
        opt = optim.AdamW(model.parameters(), lr=2e-4, weight_decay=0.02)
        ts = args.epochs * len(trl); ws = 5 * len(trl); sch = cosine_warmup(opt, ws, ts)
        ...
        mf1 = f1_score(ls2, ps, average="macro", zero_division=0)
        all_results.append({"acc": acc, "m_f1": mf1})
        mk = " ***" if acc >= 0.90 else (" **" if acc >= 0.88 else "")
